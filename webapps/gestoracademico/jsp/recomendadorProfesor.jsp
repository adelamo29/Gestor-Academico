...
<html>
    <head>
        <meta charset="UTF-8">
        <title> AMIUP: Recomendador Profesor </title>
    ...
    <body>
        ...
        <% List<Asignatura> preRequisitos = (List<Asignatura>) request.getAttribute("PrerrequisitosProf"); 
            
            
        String jsonPreRrequisitos = gson.toJson(preRequisitos);    
        %>
        <% List<Asignatura> postRequisitos = (List<Asignatura>) request.getAttribute("PostRequisitosProf"); %>

        <% List<Asignatura> notaHistTitulacion = (List<Asignatura>) request.getAttribute("NOTAMEDIAPORANIOTITULACIONProf");  
        
        String jsonNotaHistTitulacion = gson.toJson(notaHistTitulacion);
        %>
        
        <% List<Asignatura> participacionHistTitulacion = (List<Asignatura>) request.getAttribute("PARTICIPACIONMEDIAPORANIOTITULACIONProf");
        
        String jsonParticipacionHistTitulacion = gson.toJson(participacionHistTitulacion);    
                    
        %>
        <% List<Asignatura> asistenciaHistTitulacion = (List<Asignatura>) request.getAttribute("ASISTENCIAMEDIAPORANIOTITULACIONProf"); 
        ...
        asignaturaClick = (Asignatura)request.getAttribute("AsignaturaclickProf");

        String jsonAsignaturaClick = gson.toJson(asignaturaClick);
        %>

        <% Asignatura infoAsigAnio = new Asignatura();
        infoAsigAnio = (Asignatura)request.getAttribute("InfoAsignaturaFechaProf"); 
                ...
                <div class="hijoPrueba">
                    <%if(!postRequisitos.isEmpty()){ %> 
                        <%for(Asignatura asignatura: postRequisitos){%>
                            <a href="<%= request.getContextPath() %>/recomendadorProfesor?asignatura=<%=asignatura.getNombre()%>&id=<%=asignatura.getId()%>" class="asignaturaPrueba">
                                <h3><%=asignatura.getNombre()%></h3>
                                <%if((Double.parseDouble(asignatura.getNota())) >= 9){ %>
                                    <div class="notaPrueba" style="background-color: green;"><%= (Double.parseDouble(asignatura.getNota())) %></div>
                                <%}else if((Double.parseDouble(asignatura.getNota())) >= 7 && (Double.parseDouble(asignatura.getNota())) < 9){ %>
